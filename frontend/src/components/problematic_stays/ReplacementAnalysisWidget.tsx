...
interface ReplacementAnalysisWidgetProps {
  data: any[];
  isLoading: boolean;
}

const ReplacementAnalysisWidget: React.FC<ReplacementAnalysisWidgetProps> = ({ data, isLoading }) => {
  ...
  const prepareChartData = () => {
    ...
    const withReplacement = overview.with_replacement_count || 0;
    const withoutReplacement = overview.without_replacement_count || 0;

    return [
      { name: 'Mit Ersatz', value: withReplacement, color: '#22c55e' }, // Grün
      { name: 'Ohne Ersatz', value: withoutReplacement, color: '#ef4444' } // Rot
  ...
  const replacementRate = totalItems > 0 
    ? (chartData.find(item => item.name === 'Mit Ersatz')?.value || 0) / totalItems * 100 
    : 0;

  // Extrahiere den Vergleichswert (Durchschnitt über alle Agenturen)
  const averageRate = data[0]?.average_replacement_rate || 0;
  
  // Berechne die Differenz zum Durchschnitt
  const difference = replacementRate - averageRate;
  ...
  const TrendIndicator = () => {
    // Positiv: Mehr Ersatzbereitstellung als Durchschnitt
    if (difference > 0) {
      return (
    ...
    // Negativ: Weniger Ersatzbereitstellung als Durchschnitt
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4 h-full">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="flex flex-col justify-center">
          <div className="mb-4">
            <h4 className="text-sm font-medium text-gray-500 dark:text-gray-400">Ersatzbereitstellungsquote</h4>
            <div className="mt-1">
          ...
          <div className="mb-4">
            <h4 className="text-sm font-medium text-gray-500 dark:text-gray-400">Einsätze mit Ersatz</h4>
            <div className="mt-1">
              <span className="text-2xl font-bold text-gray-900 dark:text-white">
                {chartData.find(item => item.name === 'Mit Ersatz')?.value || 0}
              </span>
            </div>
          </div>
          
          <div>
            <h4 className="text-sm font-medium text-gray-500 dark:text-gray-400">Einsätze ohne Ersatz</h4>
            <div className="mt-1">
              <span className="text-2xl font-bold text-gray-900 dark:text-white">
                {chartData.find(item => item.name === 'Ohne Ersatz')?.value || 0}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="mt-4 text-sm text-gray-600 dark:text-gray-400">
        <p>
          {replacementRate > averageRate 
            ? 'Überdurchschnittliche Ersatzbereitstellung bei problematischen Einsätzen.'
            : replacementRate < averageRate 
            ? 'Unterdurchschnittliche Ersatzbereitstellung bei problematischen Einsätzen.'
            : 'Ersatzbereitstellung entspricht dem Durchschnitt.'}
        </p>
      </div>
    </div>
  );
...
export default ReplacementAnalysisWidget; 